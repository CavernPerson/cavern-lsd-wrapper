...
fn _before_burn(deps: Deps, info: MessageInfo, amount: Uint128) -> StdResult<CosmosMsg>{
    ...
    let lsd_amount = Decimal::from_ratio(amount, 1u128) / lsd_state.exchange_rate;
...
pub fn execute_burn(
    ...
    let transfer_message = _before_burn(deps.as_ref(), info.clone(), amount)?;
...
pub fn execute_mint(
    ...
    let lsd_state = query_lsd_state(deps.as_ref(), &lsd_contracts)?;
